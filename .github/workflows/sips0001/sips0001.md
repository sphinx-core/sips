...
Let $K$ = $Argon2ID(data∣∣salt)$ be the key derived from the concatenation of the input data and salt.

First, we apply SHA-256 on the stretched key:

  - $SHA-256(K)=H_1$

**Step 2:** SHAKE256 Hashing

$Let$ $H_1$ be the output of the SHA-256 hash function. Next, we apply the SHAKE256 function on the stretched key $𝐾$:

  - $SHAKE256(K)=H_2$

**Step 3:** Chaining Hashes Using SphinxHash

After computing $𝐻_1$​ and $𝐻_2$ we use the SphinxHash function to combine the two hashes, introducing prime constants for enhanced security. The combination uses a chaining function and iterative rounds:

  - $SphinxHash(H_1,H_2)=H$

Where $𝐻_1$ and $𝐻_2$ are the outputs of SHA-256 and SHAKE256 respectively, and $𝐻$ is the final combined hash.
