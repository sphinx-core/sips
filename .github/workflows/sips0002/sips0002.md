...
![](https://github.com/sphinx-core/sips/blob/main/.github/workflows/sips0002/HashTree%20scheme%203.png)
...
$$
isValidProof=VerifySigProof(receivedProof,𝜋)
$$
...
* $𝐻_root$= merkleRootHash(the root hash of the Merkle tree)


**Signature Verification:**

For signature verification, Charlie must verify the signature sent by Alice using the received **message** and the **public key**. The mathematical notation for verifying the signature is:

$$
isValidSig=VerifySignature(M,σ,pk,H_root)
$$
...
* 𝑀= receivedMessage
* 𝜎 = sig(the signature)
* 𝑝𝑘 = receivedPK (the public key of Alice)
* $𝐻_root$ = merkleRootHash (the root hash of the Merkle tree)